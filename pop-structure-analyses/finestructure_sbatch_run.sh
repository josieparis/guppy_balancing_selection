...
function makefile {
    pbsfile="$dir/t_${cmdfile}_cmd${filenum}.sbatch"
    echo "#!/bin/bash
#SBATCH -D . 
#SBATCH -p pq
...
#SBATCH --job-name=fs_t${cmdfile}_cmd${filenum}
#SBATCH --error=$dir/fs_t${cmdfile}_cmd${filenum}.err.txt
#SBATCH --output=$dir/fs_t${cmdfile}_cmd${filenum}.out.txt 
#SBATCH --export=All
#SBATCH --mail-type=END
#SBATCH --mail-user=jrp228@exeter.ac.uk
...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/gpfs/ts0/home/jrp228/programs/finestructure_4.1.1
...
function makepfile {
    ...
    touch $parallelfile
    echo "cd ../../" >> $pbsfile
    echo "cat $parallelfile | $parallel -j $nodes" >> $pbsfile
}
